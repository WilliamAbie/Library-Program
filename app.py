...
@app.route('/detailmajalah/<int:id>', methods=['GET','POST'])
def detailMajalah(id):
    data = db.readByIdMajalah(id)
    return render_template('detailmajalah.html', data=data)
...
@app.route('/detaildvd/<int:id>', methods=['GET','POST'])
def detailDVD(id):
    data = db.readByIdDVD(id)
    return render_template('detaildvd.html', data=data)
